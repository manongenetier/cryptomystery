...
def reculeJokerNoir(paquet):
  ...
  if(ind == 53): # Si en dernière position -> passe en 2ème position
...
def reculeJokerRouge(paquet) :
  ...
  if(ind == 53) : # Si en dernière position -> passe en 3ème position
    result = inverser(paquet, ind, 2)
  elif(ind == 52): # Si en avant dernière position -> pass en 2ème position
    result = inverser(paquet, ind, 1)
  else : # Sinon permuter avec la carte juste derrière lui
...
def simpleCoupe(paquet):
  ...
  if n == 53: # Joker en dernière position
    return paquet # rien qui change
